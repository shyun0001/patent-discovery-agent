...
1. **문서 자체의 변화가 아니라 문서가 서술하는 기술의 변화를 분석하세요.**
   "섹션 추가", "보고서 정리", "연구 구조 명확화", "내용 보강"처럼 문서 편집을 가리키는
   변화는 기술 변화가 아닙니다. 이런 항목은 change_type을 `refactor`,
   significance_score를 0.2 이하로 두세요.
2. title과 description에는 추가된 텍스트에 실제로 등장하는 **구체적 기술 요소**를 쓰세요.
   구성요소·처리 절차·알고리즘·재료·파라미터·정량 수치를 그대로 인용하십시오.
   (좋은 예: "정책 버전 검증 후 서비스 유형별 RAT 매핑을 적용하는 전송 경로 선택"
    나쁜 예: "새로운 실험 섹션 추가", "연구 구조의 명확화")
3. 추가된 내용이 실험 결과라면, 무엇을 바꿔서 어떤 지표가 얼마나 변했는지를 쓰세요.
4. 기술 요소를 특정할 수 없다면 그 항목은 아예 만들지 마세요. 빈 배열도 허용됩니다.
...
5. **발명의 대상은 문서·보고서·연구 관리 절차가 아니라 기술 그 자체입니다.**
   "연구 구조 개선 방법", "보고서 작성 방법", "실험 기록 관리 방법" 같은 후보는
   만들지 마세요. 명칭에는 해당 기술 분야의 구체적 구성요소가 들어가야 합니다.
6. 발명의 명칭과 요약에는 입력된 기술 변화에 등장한 용어(장치·신호·재료·파라미터 등)를
   사용하고, 일반적인 표현("성능 최적화", "구조 개선")만으로 채우지 마세요.
...
### 배경 기술 (background_art)
- 종래에 어떤 방식이 쓰였는지를 **사실 위주로** 서술 (문제 지적이 아님)
- problem과 같은 문장을 반복하지 마세요

### 기술적 과제 (problem)
- "종래의 ~에서는 ~하는 문제가 있었다" 형식
- background_art에 서술한 종래 방식의 **한계**에 초점을 둘 것
- 구체적인 기술적 한계를 서술
- 정량적 데이터가 있으면 포함
...
- 2~4개 명사 조합 권장
- **해당 기술 분야를 특정하는 명사가 반드시 하나 이상 포함되어야 합니다.**
  "구조 개선", "성능 최적화", "연구 관리"처럼 어느 분야에나 해당되는 일반어만으로
  검색어를 만들면 무관한 특허가 검색됩니다.
